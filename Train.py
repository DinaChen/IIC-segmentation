...
class SegmentationNet10aTwoHead(VGGNet):
    ...
    def forward(self, x):
        ...
        return self.head_A(x)[0], self.head_B(x)[0]
...
def main():
    ...
    potsdam_loader = torch.utils.data.DataLoader(potsdam, num_workers=4, batch_size=batch_size, shuffle=True)

    optimizer = optim.Adam(segModel.parameters(), lr = learningRate)

    for epoch in range(epoch_size):
        # Train the model batch by batch
        batch = 0
        for data in potsdam_loader:
            print(str(epoch)+ '########## Batch ' + str(batch + 1) + ' ######## ' + str(data.shape[0]) + ' images ##########')
            ...
            outputOrigin, outputOrigin_overCluster = segModel(originBatch)
            outputFlip, outputFlip_overCluster = segModel(flipBatch)
            outputJit, outputJit_overCluter = segModel(jitterBatch)


            # flip back the images for loss calculation
            outputFlip = torchvision.transforms.functional.hflip(outputFlip)
            outputFlip_overCluster = torchvision.transforms.functional.hflip(outputFlip_overCluster)
    ...
    for data in potsdamTest_loader:
        ...
        with torch.no_grad():
            testOutput, _ = segModel.forward(testData)
